...
| File | What it is |
|---|---|
| `index.html` | The site shell (~500KB, avatar embedded base64 ×2). Touch only to add one-line includes. |
...
| `launch-banner.js` | Self-mounting: 3D word-funnel canvas banner (robot core), $C7 + Roadmap nav links, hero $C7 button (emblem), glass CSS loader, claims-safe HUD bar. |
...
| `rwa-widget.js` | Live RWA market strip under hero (pinned CoinGecko ids; keep "Data by CoinGecko"). |
...
## 5. DESIGN SYSTEM — "crystal blue glass"
...
`/guardian-voice.js` `/guardian-video.mp4` `/cyre-token-256.png` `/cyre-token-512.png`
