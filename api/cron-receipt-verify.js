// api/cron-receipt-verify.js — FREE verify for cron receipts
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    input = String((req.query && req.query.token) || (req.headers['x-guardian-cron'] || '')).trim() || null;
  }
  ...
  if (!input) {
    return res.status(200).json({
      ...
      howTo: { online: 'GET /api/cron-receipt/verify?token=<cron token>' },
      disclaimer: DISCLAIMER
    });
  }

  const result = verifyToken(input, Date.now(), { kinds: [CRON_KIND] });
  return res.status(200).json({
    ok: true,
    valid: result.valid,
