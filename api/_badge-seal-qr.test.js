...
// Fail this test = do not ship (phone-unscannable QR).
...
const EXPECT_DIM_MIN = Math.round(SEAL_CANVAS * QR_PCT);
...
assert.ok(
  sealMeta.qr.qrDim >= EXPECT_DIM_MIN,
  `QR module field must be ≥${EXPECT_DIM_MIN}px (14%), got ${sealMeta.qr.qrDim}`
);
assert.ok(sealMeta.qr.pct + 1e-12 >= QR_PCT, `QR pct must be ≥0.14, got ${sealMeta.qr.pct}`);
...
try {
  ...
  fs.writeFileSync('/opt/cursor/artifacts/GRD-2026-00001-qr14-master.png', sealMeta.png);
  fs.writeFileSync(
    '/opt/cursor/artifacts/GRD-2026-00001-qr14-320px.png',
