...
function buildTimeline(times, now) {
  const defs = [
    { id: '1h', label: 'Last hour', seconds: HOUR },
    { id: '24h', label: 'Last 24h', seconds: DAY },
    { id: '7d', label: 'Last 7d', seconds: 7 * DAY },
    { id: '30d', label: 'Last 30d', seconds: 30 * DAY }
  ];
  const buckets = defs.map((d) => ({
    id: d.id,
    label: d.label,
    count: times.filter((t) => now - t < d.seconds).length
  }));
  buckets.push({
    id: 'window',
    label: 'In 1k-sig window',
    count: times.length
  });
  return buckets;
}

function buildBoard(address, list, bal, mintAffinity) {
  ...
  const burstTriggered = lastHour >= 40 || last24 >= 40;
  patterns.push({
    id: 'burst',
    pattern: 'burst',
  ...
  patterns.push({
    ...
    detail: affinityTriggered
      ? [holds.length ? 'Hold: ' + holds.join(', ') : null, touches.length ? 'Touch (zero balance): ' + touches.join(', ') : null]
          .filter(Boolean)
          .join(' \u00b7 ')
  ...
  patterns.push({
    ...
    evaluated: false,
    detail:
      'Insufficient data in v1 — RWA\u2194lend/borrow venue loops need instruction-level decode beyond the cheap sig + token-account window.',
    measured: null
  });
  patterns.push({
    ...
    evaluated: false,
    detail:
      'Insufficient data in v1 — transfer-hook / eligibility friction needs Token-2022 extension introspection (deferred for cost).',
    measured: null
  });
  ...
  return {
    ...
    patterns,
    mintAffinity: mintAffinity || [],
    timeline: buildTimeline(times, now),
    counters: {
